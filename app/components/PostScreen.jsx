...
export default function PostScreen({ post, brand, siblings }) {
   ...
   * reached without an await in front of it or Safari drops the gesture. Active
   * format first, the other one chained behind it so they don't fight for bandwidth. */
  useEffect(() => {
    let cancelled = false
    const first = post[fmt]?.map((s) => s.full) || []
    const otherKey = fmt === 'ig' ? 'tt' : 'ig'
    const second = post[otherKey]?.map((s) => s.full) || []
    if (!first.length) return
    prefetch(first)
      .then(() => { if (!cancelled && second.length) return prefetch(second) })
      .catch(() => { /* a failed warm-up just means the tap does the work */ })
